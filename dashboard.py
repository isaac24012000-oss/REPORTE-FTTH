...
# Sección de Detalle por Mes (Expanders Interactivos)
st.markdown("### 📋 Detalles Mes a Mes por Agente")

# Crear expanders para cada mes
for mes_nombre in meses_disponibles:
    with st.expander(f"📅 {mes_nombre.upper()} - Detalles Completos de Agentes", expanded=False):
        # Obtener datos para este mes
        leads_mes, conversion_mes = get_total_leads_and_conversion(mes_nombre)
        con_cobertura_mes = get_con_cobertura_count(mes_nombre)
        
        # Crear tabla detallada por agente
        detalle_agentes = []
        df_lista = load_lista_metas()
        
        if df_lista is not None and not df_lista.empty:
            # Filtrar asesores por mes
            df_mes_lista = df_lista[df_lista['Mes'] == mes_nombre]
            
            for _, row in df_mes_lista.iterrows():
                asesor = row['Asesor']
                meta = int(row['Meta'])
                
                # Obtener conversión por asesor
                conversion_asesor = get_conversion_asesor_mes(asesor, mes_nombre)
                
                # Contar instaladas del asesor en ese mes
                df_drive = load_drive_data()
                if df_drive is not None and not df_drive.empty:
                    if 'MES' in df_drive.columns:
                        ...
                        if mes_num:
                            ...
                            df_mes_drive = pd.DataFrame()
                    
                    if not df_mes_drive.empty:
                        asesor_limpio = asesor.strip()
                        nombres_alt = get_nombres_alternativos(asesor_limpio)
                        df_mes_drive['ASESOR'] = df_mes_drive['ASESOR'].astype(str).str.strip()
                        df_asesor = df_mes_drive[df_mes_drive['ASESOR'].isin(nombres_alt)]
                        
                        instaladas_asesor = len(df_asesor[df_asesor['ESTADO'] == 'INSTALADO'])
                        canceladas_asesor = len(df_asesor[df_asesor['ESTADO'] == 'CANCELADO'])
                        pendientes_asesor = len(df_asesor[df_asesor['ESTADO'] == 'PENDIENTE'])
                        
                        cumplimiento_asesor = int((instaladas_asesor / meta * 100)) if meta > 0 else 0
                    else:
                        instaladas_asesor = 0
                        canceladas_asesor = 0
                        pendientes_asesor = 0
                        cumplimiento_asesor = 0
                else:
                    instaladas_asesor = 0
                    canceladas_asesor = 0
                    pendientes_asesor = 0
                ...
                detalle_agentes.append({
                    'Agente': asesor,
                    'Meta': meta,
                    'Instaladas': instaladas_asesor,
                    'Canceladas': canceladas_asesor,
                    'Pendientes': pendientes_asesor,
                    'Cumpl%': cumplimiento_asesor,
                    'Conv%': conversion_asesor
                })
        
        # Crear DataFrame y mostrar tabla
        if detalle_agentes:
            df_detalle = pd.DataFrame(detalle_agentes)
            
            # Crear HTML para tabla detallada
            html_detalle = '<div class="resumen-tabla"><table><thead><tr>'
            html_detalle += '<th>Agente</th><th>Meta</th><th>Instaladas</th><th>Canceladas</th><th>Pendientes</th><th>Cumpl%</th><th>Conv%</th>'
            html_detalle += '</tr></thead><tbody>'
            
            for _, row in df_detalle.iterrows():
                # Aplicar color según cumplimiento
                cumpl_color = '#10b981' if row['Cumpl%'] >= 100 else '#f59e0b' if row['Cumpl%'] >= 75 else '#ef4444'
                conv_color = '#10b981' if row['Conv%'] >= 100 else '#f59e0b' if row['Conv%'] >= 75 else '#ef4444'
                
                html_detalle += f'<tr>'
                html_detalle += f'<td style="font-weight: 600;">{row["Agente"]}</td>'
                html_detalle += f'<td style="text-align: center;">{row["Meta"]}</td>'
                html_detalle += f'<td style="text-align: center;">{row["Instaladas"]}</td>'
                html_detalle += f'<td style="text-align: center;">{row["Canceladas"]}</td>'
                html_detalle += f'<td style="text-align: center;">{row["Pendientes"]}</td>'
                html_detalle += f'<td style="text-align: center; color: {cumpl_color}; font-weight: 700;">{row["Cumpl%"]}%</td>'
                html_detalle += f'<td style="text-align: center; color: {conv_color}; font-weight: 700;">{row["Conv%"]}%</td>'
                html_detalle += f'</tr>'
            
            html_detalle += '</tbody></table></div>'
            st.markdown(html_detalle, unsafe_allow_html=True)
        else:
            st.info("No hay datos disponibles para este mes")
